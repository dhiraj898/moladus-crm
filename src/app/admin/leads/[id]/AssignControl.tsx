...
import { assignLead } from '@/features/rbac/actions'

/**
 * Manual assignment control for the lead detail page (spec §9.2 / plan Task
 * 6.4). Rendered only when the current user has `leads.edit`. A labelled select
 * of assignable users (plus an "Unassigned" option → `null`) that calls
 * `assignLead` inside a transition and surfaces `ActionResult` errors inline.
